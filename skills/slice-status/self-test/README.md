...
`ASK-DONT-GUESS` / `NO-STEP-RECORD` / `DRIFT-NOTED`).
...
| `04-disagreeing-sources-named/` | 3 slice files, 2 roadmap phases (**the real edd-loop case**) | names the disagreement in Notes instead of picking a winner | `DISAGREEMENT-NAMED` |
...
`01` is the single walking-skeleton scenario. The **safety core** is `02`, `03`, `07`, `08` — the bug
classes that ship silently because their output is indistinguishable from a correct answer: a table
built from the wrong source, missing knowledge reported as known absence, fabricated structure, and
unreported drift.

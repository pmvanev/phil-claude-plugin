...
3. **Links that resolve.** Absolute URLs for files, because GitHub emits relative paths verbatim and
   ...
   markdown link freezes both.
4. **Correct labels**, with single-valued families carrying exactly one value.
...
| A single-valued label family carrying two values | Session scratch in the body |
| A missing cross-reference whose target is unambiguous | A second copy of forge-kept state |
